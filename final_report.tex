...
To train the model, we divided the dataset into training and testing data, and we repeated the process several number of times with different training and testing points, so that each data observation is used in the training process. We combined the results and took the average of the predictions. The results are shown in section 4.
...
We analyzed the data from multiple perspectives to perform our clusterings. For all of them we used a K-means approach to create a predefined amount of groupings. 
...
\begin{figure}[h]
	\includegraphics[width=\linewidth]{"./pictures/industry employment trend"}
	\caption{Industry Trends by Year}
	\label{fig:Yearly Growth}
\end{figure}
